...
- `WeaselStats` 负责设备标识读取、计数、SQLite 持久化、同步合并和报表窗口。
...
- 数据按日期和设备保存，适合长期累计。
- `WeaselStats` 从用户数据目录的 `installation.yaml` 读取 `installation_id`；文件不存在或字段无效时使用 `unknown`，并低频重试读取。
